...
####  👉 Loop Abstraction (Selective Widening + Narrowing)
We design the loop abstraction by detecting when the program is "spinning" in a loop and generalizing (widening) the changing variables in the loop body to represent "all possible iterations" simultaneously. Such design allows us to find bugs that occur deep inside loops without actually running all iterations in the loop. 


---
👉 **The Domains**
We distinguish between the **Concrete Domain** (what happens in reality) and the **Abstract Domain** (how the analyzer represents it).


**A. Concrete Domain ($D^\flat$)**
* **Values ($V^\flat$):** The set of all actual run-time values (Integers, Strings, Object References, Null).
    * $V^\flat = \mathbb{Z} \cup \mathbb{S} \cup Obj \cup \{null\}$
* **Environment ($\rho^\flat$):** A mapping from variable names ($Vars$) to concrete values.
    * $\rho^\flat: Vars \to V^\flat$
* **State ($\Sigma^\flat$):** A tuple of the Program Counter ($pc$) and the Environment. (Per-instruction abstraction)
    * $\sigma^\flat = \langle pc, \rho^\flat \rangle$

**B. Abstract Domain ($D^\sharp$)**
* **Lattice:** This abstraction is based on a [flat lattice](https://www.cse.iitm.ac.in/~rupesh/teaching/pa/jan17/scribes/0-cp.pdf) or [constant propagation](https://homes.cs.washington.edu/~bodik/ucb/cs264/lectures/4-chaotic-notes.pdf#page=1.80) lattice. 
* **Values ($V^\sharp$):** The set of concrete values plus a special **Top** element ($\top$).
    * $V^\sharp = V^\flat \cup \{\top\}$
    * $\top$ means "Any Value" (Unknown).
    * **Partial Order ($\sqsubseteq$):**
        * $\forall v \in V^\flat, v \sqsubseteq v$ (Reflexivity)
        * $\forall v \in V^\flat, v \sqsubseteq \top$ (Top is the superset of any specific value)
        * $v_1 \not\sqsubseteq v_2$ if $v_1 \neq v_2$ (Concrete values are incomparable).
* **Environment ($\rho^\sharp$):** A mapping from variables to abstract values.
    * $\rho^\sharp: Vars \to V^\sharp$


---
👉 **The Abstraction Operations - Widening Operator ($\nabla$)**
An abstraction function $\alpha$ maps concrete to abstract. In our loop abstraction, this is actually a **Widening Operator ($\nabla$)** applied dynamically, based on a set of modified variables, $M$.

Let $M_{pc} \subseteq Vars$ be the set of variables modified in the loop starting at $pc$. The Abstraction Operation (Widening) $\nabla$ takes an abstract environment $\rho^\sharp$ and applies it at a loop header: $$ \nabla(\rho^\sharp, pc) = \rho'_{new} $$
Defined as: $$
\rho'_{new}(x) =
\begin{cases}
    \top & \text{if } x \in M_{pc} \quad \text{(Variable is modified in loop)} \\
    \rho^\sharp(x) & \text{if } x \notin M_{pc} \quad \text{(Variable is invariant)}
\end{cases} $$
This preserves precision for loop-invariant variables while forcing termination for loop-variant variables.


---
👉 **The Step Functions**
The step function determines how the program moves from one state to the next.


**A. Concrete Step Function ($F$)**
This is the standard operational semantics of the language (e.g., Python/Java bytecode execution). $$ F: \Sigma^\flat \to \mathcal{P}(\Sigma^\flat) $$Example:
* Instruction: `i = i + 1`
* Input: $\langle pc: 10, \{i: 5\} \rangle$
* Output: $\langle pc: 11, \{i: 6\} \rangle$


**B. Abstract Step Function ($F^\sharp$)**
The abstract step function handles $\top$ values. It must be **sound** (over-approximate). It corresponds to `step_one_func` in the code, combined with the logic in `enhanced_bounded_run`.

It is defined in two parts:
1. The Arithmetic/Logic Step ($S^\sharp$). 
	1. This lifts standard operations to handle $\top$.
		* Instruction: `i = i + 1`
		* Input: $\{i: \top\}$
		* Operation: $\top + 1$
		* Output: $\{i: \top\}$ (Because "Any Value" + 1 is still "Any Value").
2. The Control Flow/Widening Step (The Solver Loop)
	1. The total transfer function including the widening logic is: $$
F^\sharp(\langle pc, \rho^\sharp \rangle) =
\begin{cases}
    \nabla(S^\sharp(\dots), pc) & \text{if } pc \in LoopHeaders \land Count(pc) > Threshold \\
    S^\sharp(\langle pc, \rho^\sharp \rangle) & \text{otherwise}
\end{cases}
$$

---
👉 **The concretion Operation - Narrowing Operator $\Delta$**
While Widening ($\nabla$) jumps **up** the lattice to reach a safe over-approximation (often losing precision), Narrowing ($\Delta$) attempts to move **down** the lattice to regain precision without losing safety.
* **Widening:** Concrete $\to \top$ (Generalization).
* **Narrowing:** $\top \to$ Concrete (Refinement).

Standard narrowing often starts from the widened fixed-point. However, here we use a **Bounded Re-execution from a Concrete Seed**, which remembers the state of the environment *exactly before* it was destroyed by widening.

Let $\sigma^\sharp_{pre}$ be the "Pre-Widen Snapshot".
* This environment contains specific integers/objects (e.g., $\{i: 2\}$) rather than $\top$.

The narrowing process starts a fresh exploration frontier $Q_{narrow}$ initialized with this snapshot: $$ Q_{narrow} = \{ \langle pc_{header}, \sigma^\sharp_{pre} \rangle \} $$
**The Restricted Step Function ($F^\sharp_{narrow}$)**
The narrowing phase uses a modified step function that strictly **forbids back-edges**. This prevents the analyzer from re-entering the infinite loop logic that required widening in the first place.

Let $pc_{curr}$ be the current program counter and $pc_{next}$ be the next program counter. The Restricted Step Function is defined as: $$
F^\sharp_{narrow}(\langle pc_{curr}, \rho^\sharp \rangle) =
\begin{cases}
    \emptyset & \text{if } pc_{next} \le pc_{curr} \quad \text{(Back-edge detected: Stop)} \\
    F^\sharp(\langle pc_{curr}, \rho^\sharp \rangle) & \text{otherwise} \quad \text{(Normal execution)}
\end{cases}
$$
This  limits the narrowing pass to a single "unrolling" of the loop body (or `NARROW_STEPS` deep into the body), preventing non-termination.


**The Refinement Operator (Merging)**
After running the restricted step function, we have two competing states for a specific PC:
1.  **$\rho^\sharp_{wide}$**: The state from the widening pass (contains $\top$).
2.  **$\rho^\sharp_{narrow}$**: The new state from the narrowing pass (likely contains concrete values).

We merges these using a heuristic **Asymmetric Intersection**. Let $v_{wide} = \rho^\sharp_{wide}(x)$ and $v_{narrow} = \rho^\sharp_{narrow}(x)$. The refined value $v_{new}$ is: $$
v_{new} =
\begin{cases}
    v_{narrow} & \text{if } v_{wide} = \top \land v_{narrow} \neq \top \quad \text{(Precision Regained)} \\
    v_{wide} & \text{if } v_{narrow} = \top \land v_{wide} \neq \top \quad \text{(Keep existing info)} \\
    v_{narrow} & \text{otherwise}
\end{cases} $$
* In standard lattice theory, intersection ($\sqcap$) would return $\bot$ (Bottom/Impossible) if values conflict (e.g., $5 \sqcap 6 = \bot$). Our code, however, assumes the paths are valid alternatives and "prefers" the concrete value over $\top$.


---
👉 **Example Trace #1 - Widening**
> 🤖 Gemini 2.5

**Program:** `while (i < 10) { i++; k = 5; }` (Assume `k` is invariant).
1.  **Start:** $\rho^\sharp = \{i: 0, k: 5\}$
2.  **Steps 1 & 2 (Concrete-like):**
    *   $F^\sharp$ runs normally because loop count $\le$ `WIDEN_AFTER`.
    *   State reaches: $\{i: 2, k: 5\}$.
3.  **Detect Loop:**
    *   Analyzer identifies $M_{pc} = \{i\}$ (Since $i$ changed, but $k$ did not).
4.  **Abstraction Triggered:**
    *   Apply $\nabla(\rho^\sharp, pc)$.
    *   Check $i$: $i \in M_{pc} \implies i \to \top$.
    *   Check $k$: $k \notin M_{pc} \implies k \to 5$.
5.  **Resulting Abstract State:**
    *   $\rho^\sharp_{final} = \{i: \top, k: 5\}$.

| Feature           | Concrete Domain    | Abstract Domain (This Code)                                |
| :---------------- | :----------------- | :--------------------------------------------------------- |
| **Value**         | $5$                | $5$ OR $\top$                                              |
| **Environment**   | $\{x: 5\}$         | $\{x: \top\}$                                              |
| **Operation**     | $5 + 1 = 6$        | $\top + 1 = \top$                                          |
| **Control Flow**  | Iterates $N$ times | Iterates $k$ times, then Widens                            |
| **Loop Handling** | Exact unrolling    | **Selective Widening:** $\rho(x) \to \top$ iff $x$ changes |


👉 **Example Trace #2 - Narrowing & Regaining Precision**
> 🤖 Gemini 2.5
...
**A. Widening Phase (The "Blur")**
1.  The loop runs a few times.
2.  Widening triggers. $i$ becomes $\top$.
3.  The analyzer checks `if i == 0` with $i = \top$.
    *   Since $\top$ *could* be 0, it takes the `True` branch.
    *   Since $\top$ *could* be != 0, it takes the `False` branch.
4.  It merges the results. `special_flag` becomes $\top$ (because it is True on one path and False on the other).
5.  **Result:** We lost the specific knowledge about the first iteration.

**B. Narrowing Phase (The "Focus")**
1.  **Seed:** It reloads the snapshot taken just before widening. Let's say the snapshot was at $i=0$.
    *   $\rho^\sharp_{seed} = \{ i: 0 \}$.
2.  **Step 1:** Run `if i == 0`.
    *   Since $i=0$ (Concrete), the condition is strictly **True**.
    *   The `False` branch is unreachable in this specific narrowing step.
3.  **Update:** `special_flag` becomes `True`.
4.  **Refinement:**
    *   Widened State says `special_flag = \top`.
    *   Narrowed State says `special_flag = True`.
    *   **Merge:** The analyzer updates the state to `special_flag = True`.

**Effect:** Even though the loop variable $i$ was widened to infinity to handle the loop execution, the narrowing pass recovered the precise fact that `special_flag` is definitely `True` for the specific iteration captured in the snapshot.

| Feature | Widening ($\nabla$) | Narrowing (This Implementation) |
| :--- | :--- | :--- |
| **Direction** | Up ($\sqsubseteq$) | Down (Refining) |
| **Input** | Loop History | Pre-Widening Snapshot |
| **Step Logic** | Standard | Banned Back-edges |
| **Environment** | Injects $\top$ | Recovers Concrete Values |
| **Goal** | Termination | Precision (Reduce False Positives) |
