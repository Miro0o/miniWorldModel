# Formal System, Formal Semantics, and Formal Logic
...
↗ [LISP-Based Languages](../../../🔑%20CS%20Core/👩‍💻%20Computer%20Languages%20&%20Programming%20Methodology/GPL%20%28General%20Purpose%20Languages%29/LISP-Based%20Languages/LISP-Based%20Languages.md)
↗ [Expert System (ES)](../../../🧠%20Computing%20Methodologies/👽%20Artificial%20Intelligence/🗝️%20AI%20Basics%20&%20Major%20Techniques/🌌%20Knowledge%20Representation%20%28Syntax%20Level%29%20and%20Reasoning%20%28KRR%29/🦴%20Symbolic%20AI%20&%20Logic%20Programs/Expert%20System%20%28ES%29/Expert%20System%20%28ES%29.md)
...
↗ [Formal Syntax & Metasyntax (and Metalanguage)](📌%20Formal%20Syntax%20&%20Metasyntax%20%28and%20Metalanguage%29/Formal%20Syntax%20&%20Metasyntax%20%28and%20Metalanguage%29.md)
...
\end{document}
```
Also in ↗ [Computational Trilogy & Curry–Howard(–Lambek) Correspondence](../Proof%20Theory/Computational%20Trilogy%20&%20Curry–Howard%28–Lambek%29%20Correspondence.md)
...
↗ [Number Sets & Field Construction (Completion) and Extension](../🛒%20Set%20Theory%20&%20Axiomatic%20Set%20Theory/Number%20Sets%20&%20Field%20Construction%20%28Completion%29%20and%20Extension/Number%20Sets%20&%20Field%20Construction%20%28Completion%29%20and%20Extension.md)
...
- Henkin semantics & Henkin model
	...
	- ↗ [Henkin Model & Henkin Semantics](🎩%20Higher-Order%20Languages%20&%20Logics%20%28HOL%29/Lambda%20Calculus%20%28λ-Calculus%29/Henkin%20Model%20&%20Henkin%20Semantics.md)
...
> 
> The syntax of first-order predicate calculus can be described by a ==**Type-2, or context-free, grammar**==. While the core structure of predicate calculus is context-free, complications can arise depending on how specific language features are defined.
> 
> For higher-order logics, ==their syntax can largely be described by a **Type-2, or context-free, grammar**, in the same way as first-order logic==. The additional complexity of higher-order logic resides primarily in its **semantics**—the meaning of formulas and the rules of inference—not in the structure of how those formulas are written.
...
↗ [Intuitionistic (Constructive) Logic](Intuitionistic%20%28Constructive%29%20Logic/Intuitionistic%20%28Constructive%29%20Logic.md)
↗ [Modal Logic (模态逻辑)](Modal%20Logic%20%28模态逻辑%29/Modal%20Logic%20%28模态逻辑%29.md)
...
↗ [Automata Theory and (Formal) Language Theory](../😶‍🌫️%20Theory%20of%20Computation/🍏%20Automata%20Theory%20and%20%28Formal%29%20Language%20Theory/Automata%20Theory%20and%20%28Formal%29%20Language%20Theory.md)
↗ [Formal Syntax & Metasyntax (and Metalanguage)](📌%20Formal%20Syntax%20&%20Metasyntax%20%28and%20Metalanguage%29/Formal%20Syntax%20&%20Metasyntax%20%28and%20Metalanguage%29.md) ⭐
- ↗ [BNF (Backus–Naur Form)](📌%20Formal%20Syntax%20&%20Metasyntax%20%28and%20Metalanguage%29/Formal%20Syntax%20Notations/BNF%20%28Backus–Naur%20Form%29.md)
...
### Formal Semantics & Semantic Models (Structures)
> 🔗 https://thzt.github.io/2018/01/27/semantics-3/
...
2. **$I$ 是从 $\mathcal{L}$ 到 $\mathbb{M}$ 的映射**，称为**解释**，记为 $I: \mathcal{L} \to \mathbb{M}$。它满足下面三个条件：
...
> [!EXAMPLE] 例：初等算术语言 $\text{II}$ 的结构
> 我们可以指定其结构为偶对 $N = (\mathbb{N}, I)$：
> - 论域 $\mathbb{N}$ 为自然数集。
> - $I(S)$ 为自然数集上的加 1 函数。
> - $I(+)$ 为自然数加法运算。
> - $I(\cdot)$ 为自然数乘法运算。
...
#### Language (Formula) Evaluation
> 🔗 https://thzt.github.io/2018/01/27/semantics-3/

**赋值** $\sigma$ 是一个映射，$\sigma : V \to \mathbb{M}$。
它将 $\mathcal{L}$ 中的每一个变元，赋以论域 $\mathbb{M}$ 中的一个元素 $a$，记为：
$$\sigma(x) = a, \text{ 其中 } x \in V, a \in \mathbb{M} \text{}$$
...
**项的语义 (Semantics of Terms)**
在选择了模型 $(M, \sigma)$ 之后，$\mathcal{L}$ 中的合法项 $t$ 的语义，就可以归纳地定义为论域 $\mathbb{M}$ 中的元素，记为 $t_{M[\sigma]}$。
* **(1)** $x_{M[\sigma]} = \sigma(x)$，$x$ 为变元符号
* **(2)** $c_{M[\sigma]} = c_M$，$c$ 为常元符号
* **(3)** $(f t_1 \cdots t_n)_{M[\sigma]} = f_M((t_1)_{M[\sigma]}, \cdots, (t_n)_{M[\sigma]})$

> [!EXAMPLE] 例：初等算术 II 中项的语义
> $(+ x_1 S x_7)_{N[\sigma]} = (x_1)_{N[\sigma]} + (S x_7)_{N[\sigma]} = 1 + ((x_7)_{N[\sigma]} + 1) = 1 + (7 + 1) = 9$


**逻辑公式的语义 (Semantics of Formulas)**
公式 $A$ 在模型 $(M, \sigma)$ 下的语义是一个真假值，用 $A_{M[\sigma]}$ 表示，归纳定义如下：
1. $(P t_1 \cdots t_n)_{M[\sigma]} = P_M((t_1)_{M[\sigma]}, \cdots, (t_n)_{M[\sigma]})$
2. $(t_1 \doteq t_2)_{M[\sigma]} = \begin{cases} T, & \text{if } (t_1)_{M[\sigma]} = (t_2)_{M[\sigma]} \\ F, & \text{otherwise} \end{cases}$
3. $(\neg A)_{M[\sigma]} = B_{\neg}(A_{M[\sigma]})$
4. $(A \lor B)_{M[\sigma]} = B_{\lor}(A_{M[\sigma]}, B_{M[\sigma]})$
5. $(A \wedge B)_{M[\sigma]} = B_{\wedge}(A_{M[\sigma]}, B_{M[\sigma]})$
6. $(A \to B)_{M[\sigma]} = B_{\to}(A_{M[\sigma]}, B_{M[\sigma]})$
...
其中，二元真值函数 $B_{\wedge}, B_{\lor}, B_{\to}, B_{\leftrightarrow}$ 分别逻辑连接词 $\wedge, \lor, \to, \leftrightarrow$ 的语义。

至此，我们通过为一阶语言指定模型，将语言中所有的符号串都进行了解释，即，从逻辑学角度给出了语义模型的定义。由此，一阶逻辑系统中的符号串，都有了一个数学对象与之对应，它们是论域，论域集合上的函数和运算。可想而已，这些数学对象是有代数性质的，下文我们将继续深入了解。


### The (Algebraic?) Properties of Semantic Models
> [!links]
> ↗ [Algebra](../../🧊%20Algebra/Algebra.md)
...
在公理化系统中，逻辑公式 $A$ 可以用来表示推导规则的前提和结论。公式 $A$ 在给定模型 $(M, \sigma)$ 中的语义 $A_{M[\sigma]}$ 是一个真假值。这是合情合理的，因为推导本来就应该是从真命题推导出另一个真命题的过程。

由于模型是可以人为选择的，所以，给定一个逻辑公式 $A$，其语义的真假性，有可能会受到所选模型的影响。
* **定义：** 如果存在模型 $(M, \sigma)$，使得 $A_{M[\sigma]} = T$ 成立，我们就称公式 $A$ 关于模型 $(M, \sigma)$ 是**可满足的**。
...
↗ [Computation-Tree Logic (CTL*) Family](Modal%20Logic%20%28模态逻辑%29/Temporal%20Logic%20%28时态逻辑%29/Computation-Tree%20Logic%20%28CTLstar%29%20Family/Computation-Tree%20Logic%20%28CTLstar%29%20Family.md)


> 🔗 https://thzt.github.io/2018/01/30/semantics-4/

有了可满足性，就可以进行逻辑推理。
* **定义**：设 $A$ 为公式，$\Gamma$ 为公式集。如果对于任意结构 $M$ 和任意赋值 $\sigma$，只要 $M \models_{\sigma} \Gamma$ 成立，就有 $M \models_{\sigma} A$ 成立。
* **称呼**：称 $A$ 是公式集 $\Gamma$ 的**逻辑结论**或**语义结论**。
* **记号**：记为 $\Gamma \models A$，也称结论 $\Gamma \models A$ **有效**。

> [!TIP] 语义关系
> $\Gamma \models A$ 表示一种语义关系：它指出对任意 $M$ 和 $\sigma$，如果 $\Gamma$ 为真，那么 $A$ 也一定为真。

> [!TIP]
> ↗ [(Formal) Model Checking](../../../CyberSecurity/🏰%20Cybersecurity%20Basics%20&%20Information%20Security%20%28InfoSec%29/🙇‍♂️%20Formal%20Verification%20%28FV%29%20&%20Reasoning%20Systems%20%28Formal%20Methods%29/🧳%20%28Formal%29%20Model%20Checking/%28Formal%29%20Model%20Checking.md) problem: does a given model $M$ satisfy a given property $\Phi$? $$M \models \Phi \ ?$$
> 
...
### Coherence /Consistence (协调性 /一致性)


### Decidability (决定性 /判定性)
...
- ↗ [Decidability](../😶‍🌫️%20Theory%20of%20Computation/Computability%20%28Recursion%29%20Theory%20-%20Turing%20Machine%20and%20R.E.%20Language/Decidability.md)
↗ [Complexity Theory & Computational Complexity](../😶‍🌫️%20Theory%20of%20Computation/Complexity%20Theory%20&%20Computational%20Complexity/Complexity%20Theory%20&%20Computational%20Complexity.md)
...
### Soundness & Completeness (可靠性和完全性)
> [!links]
> ↗ [Logic (and Critical Thinking) /Soundness, Truth, and Completeness](../../../../Other%20Networks%20of%20Knowledge/♂%20Philosophy%20&%20Its%20History/Classical%20Philosophy/Western%20Philosophy%20&%20Its%20History/🎼%20Logic%20%28and%20Critical%20Thinking%29/Logic%20%28and%20Critical%20Thinking%29.md#Soundness,%20Truth,%20and%20Completeness)
...
> - ↗ [Natural Number & Peano Axioms](../🛒%20Set%20Theory%20&%20Axiomatic%20Set%20Theory/Number%20Sets%20&%20Field%20Construction%20%28Completion%29%20and%20Extension/Natural%20Number%20&%20Peano%20Axioms.md)
> 
> ↗ [Church–Turing Thesis (Computability Thesis)](../😶‍🌫️%20Theory%20of%20Computation/Computability%20%28Recursion%29%20Theory%20-%20Turing%20Machine%20and%20R.E.%20Language/Church–Turing%20Thesis%20%28Computability%20Thesis%29.md)
